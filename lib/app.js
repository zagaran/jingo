...
module.exports.initialize = function (config) {
  ...
  function requireAuthentication (req, res, next) {
    if (!res.locals.user) {
      var redirectPath = res.locals.proxyPath + '/login'
      if (req.originalUrl !== '/login' && req.originalUrl !== '/login/') {
        redirectPath = redirectPath + '?destination=' + encodeURIComponent(req.originalUrl)
      }

      // Look for hash
      var split_url = req.originalUrl.split("#")
      if(split_url.length > 2) {
        req.session.hash = "#" + split_url[split_url.length -1]
      } else {
